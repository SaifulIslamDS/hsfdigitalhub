...
export function readSecurityConfig() {
  ...
  if (secret === pin) {
    return {
      ok: false,
      message:
        "HSF_ACCESS_SECRET must be different from HSF_ACCESS_PIN.",
    };
  }
...
export async function pinMatches(submittedPin, configuredPin) {
...
export function safeReturnTo(rawValue) {
  ...
  // Only a same-site path is accepted. Scheme-relative and auth endpoints
  ...
  // Header-splitting / control-character protection.
...
export function protectedResponseHeaders(sourceHeaders) {
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    body{min-height:100vh;display:grid;place-items:center;padding:24px;background:
      radial-gradient(circle at 15% 10%,rgba(0,140,68,.10),transparent 30rem),
      radial-gradient(circle at 88% 85%,rgba(30,142,137,.09),transparent 30rem),
      linear-gradient(145deg,#F9FCFA,#EEF7F2)}
    ...
    .intro:after{content:"";position:absolute;width:240px;height:240px;border-radius:999px;border:46px solid rgba(0,140,68,.06);right:-96px;bottom:-88px}
    ...
    input{width:100%;height:58px;border:1px solid #CFE1D6;border-radius:15px;padding:0 18px;background:#FAFCFB;color:#163F2A;font-size:22px;letter-spacing:.42em;text-align:center;font-weight:800;outline:none;transition:.18s}
    input:focus{border-color:var(--green);box-shadow:0 0 0 4px rgba(0,140,68,.11);background:#fff}
    button{width:100%;height:52px;border:0;border-radius:14px;background:var(--green);color:#fff;font-weight:900;font-size:14px;cursor:pointer;margin-top:15px;box-shadow:0 10px 20px rgba(0,140,68,.18);transition:.18s}
    ...
    @media(max-width:780px){
      ...
      .intro{min-height:auto;padding:34px 30px}
      h1{font-size:31px;margin-top:44px}
...
<body>
  <main class="shell">
    <section class="intro" aria-label="About this protected resource">
      <div class="brand">
        <div class="mark" aria-hidden="true">HSF</div>
        <div>
          <small>Human Safety Foundation</small>
          <strong>Digital Transformation Knowledge Hub</strong>
        </div>
      </div>
      <h1>Protected institutional reference resource.</h1>
      <p>
        This demonstration site contains HSF working frameworks, design references and institutional guidance.
        Access is intended for authorized HSF reviewers and collaborators.
      </p>
      <div class="privacy">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#008C44" stroke-width="2" aria-hidden="true">
          <rect x="5" y="10" width="14" height="10" rx="2"></rect>
          <path d="M8 10V7a4 4 0 0 1 8 0v3"></path>
        </svg>
        <span>The PIN is verified by a Netlify Edge Function. It is not stored in the public HTML or client-side JavaScript.</span>
      </div>
    </section>
    ...
    <section class="panel">
      ...
      <form action="/__hsf_access" method="post">
        ...
        <input id="pin" name="pin" type="password" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" minlength="6"
               autocomplete="one-time-code" aria-describedby="pin-help" autofocus required>
        <button type="submit">Access Knowledge Hub</button>
        <div class="session" id="pin-help">Six numeric digits are required. Repeated failed attempts are rate-limited.</div>
      </form>`}
